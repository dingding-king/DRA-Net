...
class RN_B(nn.Module):
    ...
    def forward(self, x):
        ...
        mask = mask.float()  # 将 mask 转换为浮动类型
        mask = F.interpolate(mask, size=x.size()[2:], mode='nearest')   # after down-sampling, there can be all-zero mask

        rn_x = self.rn(x, mask)
        foreground_gamma = self.foreground_gamma[None, :, None, None]  # (1, 1024, 1, 1)
        foreground_beta = self.foreground_beta[None, :, None, None]  # (1, 1024, 1, 1)
        background_gamma = self.background_gamma[None, :, None, None]  # (1, 1024, 1, 1)
        background_beta = self.background_beta[None, :, None, None]  # (1, 1024, 1, 1)

        rn_x_foreground = (rn_x * mask) * (1 + foreground_gamma) + foreground_beta
        rn_x_background = (rn_x * (1 - mask)) * (1 + background_gamma) + background_beta
